 ...
 */
package com.faw.utils.io;


import com.faw.utils.collect.SetUtils;
import com.faw.utils.common.LogUtils;
import com.faw.utils.lang.ObjectUtils;
...
public class PropertiesUtils {
	...
	private static final class PropertiesLoaderHolder {
		...
		public static void releadInstance(){
			...
			Resource[] resources = ResourceUtils.getResources("classpath*:/config/impala-*.*");
...
//			configSet.add("classpath:config/jeesite.yml");
			...
			logger.debug("Loading abf config: {}", (Object)configFiles);
				...
				if (resource.exists()){
        			if (location.endsWith(".properties")){
        				...
        				try {
	            			...
	            			logger.error("Load " + LogUtils.convertValidLog(location) + " failure. ", ex);
	        			} finally {
