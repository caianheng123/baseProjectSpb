 ...
 */
package com.faw.utils.codec;


import com.faw.utils.collect.ListUtils;
import com.faw.utils.common.LogUtils;
import com.faw.utils.lang.ExceptionUtils;
	...
	public static String xssFilter(String text) {
		...
		if (text != null){
			...
			if (logger.isInfoEnabled() && !value.equals(oriValue)){
				logger.info("xssFilter: {}   <=<=<=   {}", LogUtils.convertValidLog(value), LogUtils.convertValidLog(text));
			}
			return value;
		}
		return null;
